...
#[repr(C)]
#[derive(Clone, Copy, Debug, bytemuck::Pod, bytemuck::Zeroable)]
struct ShaderUniform {      //            align(16) size(64) 
...
const VERTICES: &[Vertex] = &[
    Vertex { position: [-1.0,  1.0, 0.0], colour: [1.0, 0.0, 0.0] },
    Vertex { position: [-1.0, -1.0, 0.0], colour: [0.0, 1.0, 0.0] },
    Vertex { position: [ 1.0, -1.0, 0.0], colour: [0.0, 0.0, 1.0] },
    Vertex { position: [ 1.0,  1.0, 0.0], colour: [0.3, 0.4, 0.3] },
];
...
impl State {
    async fn new(window: Window) -> Self {
        ...
        shader_uniform.update_resolution(window.inner_size().width as f32, window.inner_size().height as f32);

        let shader_buffer = device.create_buffer_init(
    ...
    fn resize(&mut self, new_size: winit::dpi::PhysicalSize<u32>) {
        if new_size.width > 0 && new_size.height > 0 {
            ...
            self.shader_uniform.update_resolution(new_size.width as f32, new_size.height as f32);
            self.surface.configure(&self.device, &self.config);
        }
    }

    fn input(&mut self, event: &WindowEvent) -> bool {
        self.ui_wrapper.winit_ctx.on_event(&self.ui_wrapper.ctx, event).consumed;

        match event {
            WindowEvent::CursorMoved { position, ..} => {
                self.shader_uniform.mouse.update_position(position.x as f32, position.y as f32);
                self.shader_uniform.mouse.drag_mouse(self.shader_uniform.resolution, self.shader_uniform.axis_range);
                true
            }
            ...
            WindowEvent::MouseWheel {
                ...
                match delta {
                    MouseScrollDelta::LineDelta(_, y) => {
                        if *y < 0.0 { self.shader_uniform.axis_range *= 1.05 } else { self.shader_uniform.axis_range *= 0.95 }
                        true
                    }
                    _ => false
                }
    ...
    fn update(&mut self) {
        if self.shader_uniform.iterations != self.ui_wrapper.interface.iterations {
            self.shader_uniform.update_iterations(self.ui_wrapper.interface.iterations);
        }

        if self.shader_uniform.value != self.ui_wrapper.interface.value {
            self.shader_uniform.update_value(self.ui_wrapper.interface.value)
        }

        if self.shader_uniform.exponent != self.ui_wrapper.interface.exponent {
            self.shader_uniform.update_exponent(self.ui_wrapper.interface.exponent);
        }
        self.queue.write_buffer(&self.shader_buffer, 0, bytemuck::cast_slice(&[self.shader_uniform]));
    }
